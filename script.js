    ...
    lolhuman.on('group-participants-update', async(chat) => {
        try {
            ...
            if (chat.action == 'add') {
                ...
                welkam = `Bienvenid@ a ${group_info.subject}`
    ...
    lolhuman.on('chat-update', async(lol) => {
        try {
            ...
            switch (command) {
                ...
                case 'menu':
                case 'ayuda':
                if (sender.split("@")[0] != owner) return reply("Bot exclusivo de PowerMX")
                    var punya_wa = "0@s.whatsapp.net"
                    var ini_text = "powsitos.com"
                    var ini_buffer = await getBuffer("https://github.com/powermx/dl/raw/master/29370715.png")
                    const ini_csreply = {
                        contextInfo: {
				...
				case 'ping':
					  const chatsIds = await lolhuman.chats.all()
                const timestamp = speed();
                const latensi = speed() - timestamp 
                p0 =` \`\`\`Cargando Mensajes...\`\`\`
                
\`\`\` - [ Chats abiertos ]  ${totalchat.length}\`\`\`
\`\`\` - [ Smartphone ] ${lolhuman.user.phone.device_manufacturer} ${lolhuman.user.phone.device_model}\`\`\`
\`\`\` - [ Version WA ] ${lolhuman.user.phone.wa_version}\`\`\`
\`\`\` - [ Servidor ] ${lolhuman.browserDescription[0]}\`\`\`
\`\`\` - [ Navegador ] ${lolhuman.browserDescription[1]}\`\`\`
\`\`\` - [ ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / 2095 ] RAM\`\`\`

\`\`\`Velocidad: ${latensi.toFixed(4)} Segundos\`\`\``
                lolhuman.sendMessage(from, p0, text, { quoted: lol})
                    break
